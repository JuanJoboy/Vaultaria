...
{
	public class DestroyerLaserBeam : ElementalProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / DestroyerLaserHoldout.NumBeams;
		...
		private const float OuterBeamOpacityMultiplier = 0.4f;
		...
		public override void AI() {
			...
			if (Projectile.type != ModContent.ProjectileType<DestroyerLaserBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<DestroyerLaserHoldout>()) {
				Projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / DestroyerLaserHoldout.MaxCharge, 0f, 1f);
			...
			Projectile.friendly = hostPrism.ai[0] > DestroyerLaserHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - DestroyerLaserHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * DestroyerLaserHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor() {
			...
			float hue = (BeamID / DestroyerLaserHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
