...
{
	public class WarriorLaserBeam : ElementalProjectile
	{
		// A helpful math constant for performing beam angling calculations.
		private const float PiBeamDivisor = MathHelper.Pi / WarriorLaserHoldout.NumBeams;
		...
		public override void AI() {
			...
			if (Projectile.type != ModContent.ProjectileType<WarriorLaserBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<WarriorLaserHoldout>()) {
				Projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / WarriorLaserHoldout.MaxCharge, 0f, 1f);
			...
			Projectile.friendly = hostPrism.ai[0] > WarriorLaserHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - WarriorLaserHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * WarriorLaserHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor() {
			...
			float hue = (BeamID / WarriorLaserHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
