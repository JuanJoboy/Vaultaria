...
namespace Vaultaria.Common.Networking
{
	internal class ModNetHandler 
	{
		...
		internal static VaultPacketHandler vault = new VaultPacketHandler(vaultType);
		...
		public static void HandlePacket(BinaryReader reader, int fromWho) // This is the method called directly by the Vaultaria mod class when any packet for the mod is received.
		{
			switch (reader.ReadByte()) // Reads the first byte of data (the HandlerType written in PacketHandler.GetPacket).
			{
				...
				case vaultType: // If the first byte matches the Boss ID (1), it calls the HandlePacket method on the specific BossPacketHandler instance, passing the remaining packet data (reader) and the sender ID (fromWho).
					vault.HandlePacket(reader, fromWho);
					break;
			}
		}
	}  
